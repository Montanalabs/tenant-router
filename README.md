...
- **Closed type:** `type Decision = RouteTenant(Tenant) | RejectTenant` where `Tenant` is closed
- **Sink / capability:** `grant routeTenant` → `privileged { routeTenant(d) }`
- **Consequence axes:** Trust

## Run the demo

```sh
examples/apps/tenant-router/demo.sh
```
...
- `tenant-router_safe.wave` — the correct design.
- `tenant-router_unsafe.wave` — the tempting-but-wrong version (the negative example a model must learn to reject).
